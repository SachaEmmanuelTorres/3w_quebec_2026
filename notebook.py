...
def _():
    ...
    response = requests.get("https://www.lemonde.fr/pages/")
    soup = BeautifulSoup(response.text, 'html.parser')

    with open('lemonde.md', 'w') as f:
        for link in soup.find_all('a'):
            f.write(f"- [{link['href']}]({link['href']}) - {link.text}\n")
    return
...
def _():
    ...
    base_url = "https://www.lemonde.fr/pages/?page="
    ...
    with open('lemonde.md', 'w') as f:
        for link in soup.find_all('a'):
            f.write(f"- [{link['href']}]({link['href']}) - {link.text}\n")
        page = int(soup.select_one('.pagination a.next').text)
        if page > 1:
